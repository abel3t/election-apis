...
export class DeleteCandidateHandler implements ICommandHandler<DeleteCandidateCommand> {
  constructor(
    private readonly prisma: PrismaService
  ) {}

  async execute({ electionId, candidateId }: DeleteCandidateCommand) {
    const existedCandidate = await this.prisma.candidate.findFirst({ where: { id: candidateId, electionId } });
